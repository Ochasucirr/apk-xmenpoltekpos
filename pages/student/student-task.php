<?php 
    ...
    if ($_SESSION['role'] != "Mahasiswa") {
        ...
        $taskQuery = "SELECT T.id, T.nama, T.batas, S.id subjectID, S.nama subject, F.nilai, F.id fileID FROM tugas T join mata_kuliah S on (T.mataKuliahID=S.id) LEFT join (SELECT * FROM file WHERE file.mahasiswaNRP='$nrp') as F on (T.id=F.tugasID) where kelasID=$classID AND S.id=$subjectID ORDER BY batas DESC";
...
<body>
    <nav class="navbar navbar-expand-lg navbar-light">
        <div class="container-fluid">
            <a class="navbar-brand" href="student-home.php">X-MENTOR</a>
    ...
    <section class="task mt-4">
        ...
        <div class="mt-4 d-flex flex-wrap">
            ...
            <a
                ...
                <div class="card-task me-3 mb-4 px-4 py-4" style="background-color: #a7e0eb;">
                    <div class="label">
                        <?= $data['nilai']; ?>
                    </div>
                    <div class="row">
                        <ul>
                            <li class="mt-1" style="height: 50px;">
                                <h4>
                                    <?= $data['nama']; ?>
                                </h4>
                            </li>
